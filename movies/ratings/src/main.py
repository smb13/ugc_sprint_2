...
async def lifespan(_: FastAPI) -> AsyncGenerator:
    ...
    await mongo.mongo[settings.mongo_db][settings.mongo_rating_collection].create_index(
        ['movie_id', 'user_id'], unique=True, background=True
    )
    await mongo.mongo[settings.mongo_db][settings.mongo_review_collection].create_index(
        ['movie_id', 'user_id'], unique=True, background=True
    )
    await mongo.mongo[settings.mongo_db][settings.mongo_bookmarks_collection].create_index(
